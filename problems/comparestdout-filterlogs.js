...
const checkSenecaLogs = chunk => {
  try {
    ...
    if (line.level) {
      // we assume seneca log has this prop
      return false
    }
  } catch (e) {
    //do nothing
...
function processor(mode, callback) {
  ...
  function transform(chunk, enc, callback) {
    if (line === 1) {
      ...
      if (!this.longCompareOutput) {
        outputStream.push(
          chalk.yellow(
            center(this.__('compare.actual'), 40) +
              center(this.__('compare.expected'), 40) +
              '\n'
          )
        )
      }
  ...
  function flush(_callback) {
    ...
    this.emit(
      equal ? 'pass' : 'fail',
      this.__(equal ? 'compare.pass' : 'compare.fail')
    )
    _callback(null)
  ...
  tuple(
    this.submissionStdout.pipe(split()).pipe(filter.obj(checkSenecaLogs)),
