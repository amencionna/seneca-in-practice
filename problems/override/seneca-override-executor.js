...
seneca.act({ role: 'math', cmd: 'sum', left: 'michele', right: 2.5 }, function(
  err
) {
  if (
    err &&
    err.toString().indexOf('Expected left and right to be numbers.') > 0
  ) {
...
seneca.act(
  {
    role: 'math',
    cmd: 'sum',
    left: parseInt(process.argv[3]),
    right: parseInt(process.argv[4])
  },
  function(err, result) {
